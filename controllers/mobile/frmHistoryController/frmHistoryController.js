...
define({ 
  ...
  */
  onSelectingDPR : function() {
    try{
      ...
      if(pending){
        var employeeData = {"empName": oceaneering.oms.appGlobals.employeeFullName, "empId" : oceaneering.oms.appGlobals.employeeId , role : ""};
        var contextData = {"projectData" : {"projectId": data.dprheader.str_projid },  "navigatingFrom": "frmHistory", "operation": "History", "employeeData": employeeData, "dprData": data.dprData , headerData : data.dprheader}; 
        if(!data.type){
          employeeData.role = "supervisor";
          formStack.push({frm : "frmHistory" , params : {isBack : true}});
        ...
        var employeeDataDone = {"empName": oceaneering.oms.appGlobals.employeeFullName, "empId" : oceaneering.oms.appGlobals.employeeId , role : ""};
        var contextDataDone = {"projectData" : {"projectId": data.dprheader.str_projid },  "navigatingFrom": "frmHistory", "operation": "View", "employeeData": employeeDataDone, "dprData": data.dprData , headerData : data.dprheader}; 
        if(!data.type){
  ...
  */
  onClickOfBack : function(){
    this.showLoadingIndicator();
    commonUtils.navigateTo("frmDashboard" , { "isBack" : false , "loadProjects" : true});
    formStack = [];
      ...
      if(!commonUtils.checkIfEmpty(history[1]) && (history[1]).length > 0){
        ...
        for(var j = 0; j < perData.length; j++){ 
          ...
          var res = {
            ...
            "lblStatus" : {"skin" : skinval , "text" : textval}, 
            "flxSign" : {
              isVisible : signPer,
