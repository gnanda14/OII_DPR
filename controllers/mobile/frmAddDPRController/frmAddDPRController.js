...
define({
  ...
  onMenuClickBody: function (input) {
    try {
      var self = this;
      var arr = ["flxPersonnelBody", "flxEventsBody", "flxRoasterBody", "flxAssetToolsBody",
                 "flxAttachmentBody", "flxCommentsBody", "flxCondition", "flxStatusBody", "flxFutureBody"
                ];
  ...
  openFileUploadPopUp: function () {
    ...
    //#ifdef iphone
    this.view.flxFile.width = "0%";
    this.view.flxFile.setVisibility(false);
    this.view.flxGallary.width = "42%";
    this.view.flxCamera.width = "42%";
  ...
  onClickOfCamera : function (){
    try {
      //#ifdef iphone
      this.view.flxEditImage.setVisibility(false);
      //#endif
  ...
  onClickOfOpenGallery : function () {
    try {
      ...
      //#ifdef iphone
      var returnStats = kony.phone.openMediaGallery(this.onImageSelectionCallbackIos.bind(this), queryContext);
      //#endif
  ...
  onClickOfOpenFileSystem : function () {
    try {
      ...
      //#ifdef iphone
      var returnStats = kony.phone.openMediaGallery(this.onFileSelectionCallbackIos.bind(this), queryContext);
      //#endif
    } catch (err) {
      commonUtils.exception(this, err);
    }
  ...
  bindAttachments : function () {
    ...
    //#ifdef iphone
    this.view.CameraImage.onCaptureFailed = this.onFailureCamera.bind(this);
    this.view.CameraImage.onImageSaveFailed = this.onFailureCamera.bind(this);
    //#endif
    ...
    try {
      ...
      if(!commonUtils.checkIfEmpty(val) && val.length > 0){
        for(var j = 0; j < val.length; j++){
          ...
          //#ifdef iphone
          if(commonUtils.getFileTypeImage(val[j].str_attachmentExtension) == "pdf.png"  || (val[j].str_attachmentExtension).substring(0,5) == "image"){
            files.push(inp);
          }
          //#endif
  ...
  openPDF : function (base64String) {
    try {
      ...
      //#ifdef iphone
      commonUtils.navigateTo("frmBrowser", {
        "base64": base64String,
        "mime" : "pdf",
        "data": this.data
      });
  ...
  animateFileLoading : function(){
    ...
    transform.translate(-340, 0);
    this.view.flxAnimSubCon.animate(
      kony.ui.createAnimation({
        "0" : {
          "transform": transform,
          "stepConfig": {
